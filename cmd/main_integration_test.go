...
func (s *SpyConfigGenerator) List(
	...
	s.ListCalledTimes++
	return s.ListFunc(ctx, interfaceAddresses, allowedIPs, persistentKeepalive, dns)
}
...
func TestMain_Run(t *testing.T) {
	t.Run("test main run()", func(t *testing.T) {
		...
		spyConfigGenerator.ListFunc = func(ctx context.Context, interfaceAddresses []netip.Prefix, allowedIPs []netip.Prefix, persistentKeepalive uint16, dns []netip.Addr) ([]wireguard.Configuration, error) {
			return []wireguard.Configuration{wireguard.NewConfiguration("private_key", interfaceAddresses, dns, []wireguard.PeerConfig{wireguard.NewPeerConfig("public_key", netip.MustParseAddrPort("1.1.1.1:51820"), []netip.Prefix{netip.MustParsePrefix("0.0.0.0/0")}, 25)})}, nil
		}
		tempDir := t.TempDir()
		app := &App{}
