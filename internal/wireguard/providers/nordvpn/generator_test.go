...
func TestConfigGenerator_(t *testing.T) {
	t.Run("happy path table tests", func(t *testing.T) {
		...
		mockPrivateKeyServer := httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
			rw.WriteHeader(http.StatusOK)
			rw.Write([]byte(`{"nordlynx_private_key":"test_key"}`))
		}))
		defer mockPrivateKeyServer.Close()

		mockServerListServer := httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
			rw.WriteHeader(http.StatusOK)
			rw.Write(
				[]byte(
					fmt.Sprintf(
		...
		tests := []struct {
		...
		}{
			{
				...
				interfaceAddresses: []netip.Prefix{netip.MustParsePrefix("10.5.0.2/32")},
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				assert.Equal(t, 0, config.Peers[0].Endpoint.Compare(expectedIpPort))
				assert.Equal(t, expectedPublicKey, config.Peers[0].PublicKey)
				assert.True(t, slices.Equal(tt.allowedIPs, config.Peers[0].AllowedIPs))
				assert.Equal(t, tt.persistentKeepalive, config.Peers[0].PersistentKeepalive)
				assert.True(t, slices.Equal(tt.dns, config.DNS))
				assert.True(t, slices.Equal(tt.interfaceAddresses, config.InterfaceAddresses))
			})
		}
	})
}
